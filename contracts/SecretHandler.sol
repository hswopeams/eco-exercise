...
contract SecretHandler is Killable, EIP712("SecretHandler", "1") {
     ...
     * Several components are used in the hash to mininimize the possibility of collisions with other parties or contract instances using the same plainMessage.
     ...
     */
    function hashSecret(bytes32 plainMessage, bytes32 salt) public view returns (bytes32 hashedSecret) {
        require(salt != bytes32(0), INVALID_SALT);
        require(plainMessage != bytes32(0), INVALID_MESSAGE);

        hashedSecret = keccak256(abi.encodePacked(address(this), plainMessage, salt));
    }
