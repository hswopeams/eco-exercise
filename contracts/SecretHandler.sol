...
contract SecretHandler is Killable, EIP712("SecretHandler", "1") {
    ...
    /**
     * @notice Commits a secret that has been agreed upon by two parties. The caller is assumed to be party 1 to the agreemtn. The transaction must be signed by
     * the party 2.
     * @dev Verifies that party2 is the signer of the transaction.
     * 
    ...
    function commitSecret(bytes32 hashedSecret, address party2, bytes32 sigR, bytes32 sigS, uint8 sigV) external { 
        require(hashedSecret !=  bytes32(0), INVALID_MESSAGE);
    ...
    /**
     * @notice Reveals secret if called by one of the two parties to the secret and not called in a block later than the commit transaction block.
     ...
     * @param salt - most likely generated by the front-end
    ...
    function revealSecret(bytes32 plainSecret, bytes32 salt, uint secretId) external {
        ...
        require(plainSecret != bytes32(0), INVALID_MESSAGE);
        require(secretId >= 1, INVALID_SECRET_ID);
        require(hashSecret(secret.party1, secret.party2, plainSecret, salt) == secret.message, SECRETS_DO_NOT_MATCH);
    ...
    /**
     * @notice Generate hash of plain text secret + party addresses + a salt passed in + contract address. This function keeps the secret private onchain.
     * Several components are used in the hash to mininimize the possibility of collisions with other parties or contracts using the same plainMessage.
     * @param party1 - party1's address
     * @param party2 - party2's address
     * @param plainMessage - the plaintext secret message
     * @param salt the party's salt
     * @return hashedSecret the hashed secret
     */
    function hashSecret(address party1, address party2, bytes32 plainMessage, bytes32 salt) public view returns (bytes32 hashedSecret) {
        require(salt != bytes32(0), INVALID_SALT);
        require(party1 != address(0) && party2 != address(0), INVALID_ADDRESS);
        require(plainMessage != bytes32(0), INVALID_MESSAGE);

        hashedSecret = keccak256(abi.encodePacked(address(this), party1, party2, plainMessage, salt));
    }
