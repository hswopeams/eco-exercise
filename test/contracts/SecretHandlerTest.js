...
describe("SecretHandler", function () {
  ...
  describe("commitSecret() ", function () {
    ...
    context("Reverts", function () {
      ...
      it("should revert with the right error if secret message is invalid", async function () {
        ...
        ).to.be.revertedWith(RevertReasons.INVALID_SECRET);
      });
      ...
      it("should revert with the right error if party1 signed the transaction", async function () {
        ...
        await expect(
          secretHandler
            .connect(party1)
            .commitSecret(hashedSecret, party2.address, splitSignature.r, splitSignature.s, splitSignature.v)
        ).to.be.revertedWith(RevertReasons.SIGNER_AND_SIGNATURE_DO_NOT_MATCH);
      });
      ...
      it("should revert with the right error if recovered signer is the zero address", async function () {
        await expect(
          secretHandler
            .connect(party1)
            .commitSecret(hashedSecret, party2.address, splitSignature.r, splitSignature.s, "0") // Invalid v signature component
        ).to.be.revertedWith(RevertReasons.ECDSA_INVALID_SIGNATURE);
      });
    ...
    context("Events", function () {
      it("should emit an event on commit if hashed secret represents text", async function () {
        const tx = await secretHandler
          .connect(party1)
          .commitSecret(hashedSecret, party2.address, splitSignature.r, splitSignature.s, splitSignature.v);
        ...
        await expect(tx).to.emit(secretHandler, "SecretCommitted").withArgs(id, secretStruct, party1.address);
      });

      it("should emit an event on commit if hashed secret represents a number", async function () {
        ({ hashedSecret, salt } = await getHashedSecret(ethers.constants.MaxUint256, secretHandler));
    ...
    context("State", function () {
      it("should change state correctly if hashed secret represents text", async function () {
        ...
        // Check that secret is stored correctly. Returned values should match expected
      ...
      it("should change state correctly if hashed secret represents a number", async function () {
        ({ hashedSecret, salt } = await getHashedSecret(ethers.constants.MaxUint256, secretHandler));
        ...
        // Set message
        expectedSecret.message = hashedSecret;
  ...
  describe("revealSecret() ", function () {
    ...
    context("Reverts", function () {
      it("should revert with the right error if secretId is invalid", async function () {
        await expect(
          secretHandler.connect(party1).revealSecret(secretBytes32, salt, BigNumber.from(10))
        ).to.be.revertedWith(RevertReasons.INVALID_SECRET_ID);
      });

      it("should revert with the right error if caller is not party to the secret", async function () {
        await expect(
          secretHandler.connect(rando).revealSecret(secretBytes32, salt, emittedSecretId)
        ).to.be.revertedWith(RevertReasons.CALLER_NOT_PARTY);
      });

      it("should revert with the right error if salt is invalid", async function () {
        await expect(
          secretHandler.connect(party1).revealSecret(secretBytes32, ethers.utils.hexZeroPad("0x", 32), emittedSecretId)
        ).to.be.revertedWith(RevertReasons.INVALID_SALT);
      });

      it("should revert with the right error if secret is invalid", async function () {
        await expect(
          secretHandler.connect(party2).revealSecret(ethers.utils.hexZeroPad("0x", 32), salt, emittedSecretId)
        ).to.be.revertedWith(RevertReasons.INVALID_SECRET);
      });
      ...
      it("should revert with the right error if revealed secret doesn't match committed secret -- different secret message", async function () {
        ...
        await expect(
          secretHandler.connect(party2).revealSecret(differentSecretBytes32, salt, emittedSecretId)
        ).to.be.revertedWith(RevertReasons.SECRETS_DO_NOT_MATCH);
      });
      ...
      it("should revert with the right error if revealed secret doesn't match committed secret -- different salt", async function () {
        ...
        await expect(
          secretHandler.connect(party2).revealSecret(secretBytes32, differentSalt, emittedSecretId)
        ).to.be.revertedWith(RevertReasons.SECRETS_DO_NOT_MATCH);
      });
      ...
      it("should revert with the right error if paused", async function () {
        await secretHandler.connect(owner).pause();
        await expect(
          secretHandler.connect(party2).revealSecret(secretBytes32, salt, emittedSecretId)
        ).to.be.revertedWith(RevertReasons.PAUSED);
      });
    });
  ...
  describe("pause() ", function () {
    context("Reverts", function () {
      it("should revert with the right error if caller is not owner", async function () {
        await expect(secretHandler.connect(rando).pause()).to.be.revertedWith(RevertReasons.NOT_OWNER);
      });

      it("should revert with the right error if already paused", async function () {
        await secretHandler.connect(owner).pause();
        await expect(secretHandler.connect(owner).pause()).to.be.revertedWith(RevertReasons.PAUSED);
      });
    });

    context("Events", function () {
      it("should emit an event when owner pauses the contract", async function () {
        await expect(secretHandler.connect(owner).pause()).to.emit(secretHandler, "Paused").withArgs(owner.address);
      });
    });
  ...
  describe("unpause() ", function () {
    context("Reverts", function () {
      it("should revert with the right error if caller is not owner", async function () {
        await expect(secretHandler.connect(rando).unpause()).to.be.revertedWith(RevertReasons.NOT_OWNER);
      });

      it("should revert with the right error if not paused", async function () {
        await expect(secretHandler.connect(owner).unpause()).to.be.revertedWith(RevertReasons.NOT_PAUSED);
      });
    });
    ...
    context("Events", function () {
      it("should emit an event when owner unpauses the contract", async function () {
        await secretHandler.connect(owner).pause();
        await expect(secretHandler.connect(owner).unpause()).to.emit(secretHandler, "Unpaused").withArgs(owner.address);
      });
    });
